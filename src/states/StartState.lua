...
function StartState:init()
    ...
    self.colorTimer = Timer.every(
        0.75, function() self.colors[0] = self.colors[6] 
        for i = 6, 1, -1 do
    ...
    for i = 1, 64 do 
        table.insert(positions, gameFrames['tileFrame'][math.random(18)][math.random(6)])
    end
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            if self.currentMenuItem == 1 then
                Timer.tween(1, {
                    ...
                    gameStateMachine:change('begin-game',  {level = 1})
...
function StartState:render()
    ...
    for y = 1, 8 do
        for x = 1, 8 do
            ...
            love.graphics.draw(gameTextures['tiles'], positions[(y - 1) * x + x], 
